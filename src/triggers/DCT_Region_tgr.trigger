trigger DCT_Region_tgr on DCTRegion__c (before insert, before update) {
	...
	if (Trigger.isBefore){
		...
		if (Trigger.isInsert || Trigger.isUpdate){
	        ...
	        for(DCTRegion__c pct : Trigger.new) {
				System.debug('EN DCT_Region_tgr isAfter pct: ' + pct);	        	
        		String sIdExterna = pct.RFC__c + '-' +  pct.LegalEntity__c + '-' +  pct.Region__c;
				System.debug('EN DCT_Region_tgr isAfter sIdExterna: ' + sIdExterna);
        		pct.IdExterno__c = sIdExterna;
